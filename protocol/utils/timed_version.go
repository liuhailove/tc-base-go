...
func timeVersionComponents(v uint64) (ts int64, ticks int32) {
	return int64(v>>ticks) + epoch, int32(v & tickMask)
}
...
func NewTimedVersionFromProto(proto *tc.TimedVersion) *TimedVersion {
...
func TimedVersionFromProto(proto *tc.TimedVersion) TimedVersion {
...
func (t *TimedVersion) ToProto() *tc.TimedVersion {
	ts, ticks := timeVersionComponents(t.v.Load())
	return &tc.TimedVersion{
		UnixMicro: ts,
		Ticks:     ticks,
	}
}
...
func (t *TimedVersion) Time() time.Time {
	ts, _ := timeVersionComponents(t.v.Load())
