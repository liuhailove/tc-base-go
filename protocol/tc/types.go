...
func RoomNameAsStrings(roomNames []RoomName) []string {
...
func StringsAsRoomName(roomNames []string) []RoomName {
...
// Guid 这个接口的定义表明，任何实现了 Guid 接口的类型都必须同时满足 TrackID、
// ParticipantID 和 RoomID 三个类型。换句话说，
// Guid 接口的实现类型必须能够被视为 TrackID、ParticipantID 和 RoomID 三种类型中的任意一种。
type Guid interface {
